...
всё ещё висеть на площадке из-за сбоя API или неполного снятия (сбой записи
остатка).
...
карточка, видимая покупателям с нулевым остатком, выглядела бы «снятой».
...
Цена отказа от WITHDRAWN: если владелец снял книгу вручную и сбой записи
остатка не повторился, карточка может остаться видимой с остатком. Купить такую
книгу с нулевым остатком нельзя, так что это косметика — а вот обнулить остаток
живой книги значит снять с продажи товар, который продаётся.
"""
...
def reconcile_withdrawn_books(db: Session, marketplace: str, verbose: bool = True) -> dict:
    ...
    # не показатель: у проданной книги он 0 ещё до отгрузки (резерв под заказ).
    ...
    for key in targets:
        ...
        # остаток без архивации (архив Ozon мы не используем). withdraw_book сам проставляет статус лота:
