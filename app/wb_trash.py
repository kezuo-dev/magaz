...
#
# 0 дней: свежие продажи встают в очередь корзины сразу, без ожидания окна
# отмены. Восстановление из корзины (WB /content/v2/cards/recover) работает и
# проверено: при отмене заказа программа сама достаёт карточку обратно (см.
# process_cancelled_orders), поэтому «убрать сразу» безопасно. Признак «снят
# не этой продажей» (_withdrawn_by_order) защищает от восстановления чужих лотов.
CANCEL_GRACE_DAYS = 0
...
#
# 12 минут вместо 20: замеры на проде показали, что после 429 удачный запрос
# появляется в течение ~10-15 минут (цикл троттлинга WB на delete/trash). Слишком
# длинное окно (20) просто простаивало между удачами; 12 — чаще пробуем и не
# отстаём от момента, когда лимит отпускает.
RETRY_AFTER_429_MIN = 12
