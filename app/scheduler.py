...
def cleanup_wb_trash() -> None:
    ...
    Обрабатывает до MAX_BOOKS_PER_RUN книг (100) за проход, новые первыми.
    ...
    Часто (10 минут), но с окном тишины внутри: пока WB держит лимит (429),
    прогон выходит без единого запроса. Как только лимит отпускает — ближайший
    прогон удаляет до 100 карточек (пачками по 10). Частые прогоны нужны, чтобы
    не терять момент, когда WB готов принять запрос: она держит лимит десятки
    минут, но окна «отпустило» короткие и заранее неизвестны.
    """
    db = SessionLocal()
    try:
        if not is_sync_enabled(db):
